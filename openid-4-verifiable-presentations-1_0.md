...
The validity of data presented in the Credential(s) is attested by the Credential Issuer.

The Verifier MUST validate the integrity, authenticity, and Holder Binding of any Verifiable Presentation provided by the Wallet according to the rules of the respective Presentation format. 

The Verifiers MUST consider that presented Credential(s) can be revoked and that user data may change over time. Such changes can only be noticed by the Verifier if the Credential(s) are checked at each interaction with the End-User.
