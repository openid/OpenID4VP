...
* `issuer_signed_alg_values`: OPTIONAL. A JSON array containing cryptographic algorithms identifiers. The `alg` COSE header (as defined in [@!RFC8152]) of the `IssuerSigned` structure of the presented mdoc MUST match one of the array values, when the `IssuerSigned` structure is signed with one of a fully-specified cryptographic algorithm defined in [@!I-D.ietf-jose-fully-specified-algorithms] or with  signature algorithms defined in section 9.1.2.4 of [@ISO.18013-5]. In all other cases (commonly known as a polymoprhic cryptographic algorithm identifier), the fully-specified algorithm derived from the combination of the `alg` value from the COSE header and the `crv` parameter from the signing key MUST match one of the array values.
* `device_signed_alg_values`: OPTIONAL. A JSON array containing cryptographic algorithms identifiers. The `alg` COSE header (as defined in [@!RFC8152]) of the `DeviceSigned` structure of the presented mdoc MUST match one of the array values, when the `DeviceSigned` structure is signed with one of a fully-specified cryptographic algorithm defined in [@!I-D.ietf-jose-fully-specified-algorithms] or with  signature algorithms defined in Section 9.1.3.6 of [@ISO.18013-5]. The `alg` COSE header MUST be `HMAC 256/256` when mdoc MAC Authentication defined in Section 9.1.3.5 of [@ISO.18013-5] is used. In all other cases (commonly known as a polymoprhic cryptographic algorithm identifier), the fully-specified algorithm derived from the combination of the `alg` value from the COSE header and the `crv` parameter from the signing key MUST match one of the array values.
