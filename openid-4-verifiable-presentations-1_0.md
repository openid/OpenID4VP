...
* `well-known`: When the Client Identifier Scheme is `well-known`, the Client Identifier MUST be a case-sensitive URI using the HTTPS scheme that contains the scheme, host, and optionally, port number and path components, but no query or fragment components. The Verifier publishes their Verifier Metadata available at the path formed by concatenating the string `/.well-known/verifier-metadata` to the `client_id` value in the request. If the `client_id` value contains a path component, any terminating `/` MUST be removed before inserting `/.well-known/` and the well-known URI suffix between the host component and the path component. The Request Object MUST be signed by the Verifier with a private key corresponding to a public key in the Verifier Metadata (see (#client_metadata)). The Wallet MUST obtain the Verifier Metadata from the well-known location using an HTTP GET request and validate the signature with one of the public keys contained in the Verifier Metadata. The `iss` claim value in the Request Object MUST match the `client_id` value.

To use `client_id_scheme` values `well-known`, `entity_id`, `did`, `verifier_attestation`, `x509_san_dns`, and `x509_san_uri`, Verifiers MUST be confidential clients. This might require changes to the technical design of native apps as such apps are typically public clients.
