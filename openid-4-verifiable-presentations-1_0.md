...
The following is a non-normative example of a response using the `presentation_submission` and `vp_token` values from (#jose_cose_sdjwt_vc). (line breaks for display purposes only):
