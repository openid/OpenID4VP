...
This specification defines an additional In-band Registration mechanism to allow a Client to register with the Wallet with the Authorization Request and optionally provide their Verifier Metadata using the `client_metadata` request parameter.

Existing mechanisms such as Dynamic Client Registration [@!RFC7591] and OpenID Federation [@!OpenID.Federation] are still applicable.

To allow In-band Registration mechanism as well as existing mechanisms to be supported in parallel, Wallets and Wallet Providers MUST enforce sufficient randomness of the `client_id` for pre-registered Clients.

Wallets MUST verify the integrity of the `client_id`, `redirect_uri` and/or `response_uri` parameter from the request. Additionally, Wallets SHOULD verify the `redirect_uri` and/or `response_uri` belong to the Client with the identified by the `client_id`.
...
* Decentralized Identifier (DID): If supported by the Wallet and the request is signed, the signature of the signed request MUST be verified using a public key obtained from a `verificationMethod` property of a DID Document. Since a DID Document can include multiple public keys, the particular public key used to sign the request MUST be identified by the `kid` header referring to a `verificationMethod.id` DID URL. To obtain the DID Document, the Wallet MUST use DID Resolution as defined by the DID method used by the Verifier. If either of the checks fail, the Wallet MUST reject the request, otherwise processing continues.

This specification defines the following mechanisms for In-band Registration for unsigned requests:
* Client ID matches the Redirect and/or Response URI: If supported by the Wallet, and the request was not received via the Digital Credentials API as defined in Appendix A, the Wallet MUST verify the `client_id` request parameter equals the `redirect_uri` and/or `response_uri` parameters. If this check fails, the request MUST be rejected, otherwise processing continues.

Other specifications or ecosystem regulations MAY define mechanisms or rules complementing the rules defined above, but such extensions are out of scope of this specification.

Additionally, the following existing mechanisms are defined:
* Pe-registered Clients: If supported by the Wallet, the Wallet MUST determine the pre-registered Client metadata for the `client_id` in the request. Then, the following two cases are distinguished: 1) If the request is unsigned, the Wallet MUST check if the `redirect_uri` and/or `response_uri` request parameter matches any URI of the `redirect_uris` Client metadata. 2) If the request is signed, the Wallet MUST verify the signature of the request using one of the public keys from the `jwks` parameter of the Client metadata. If 1) or 2) fails, the Wallet MUST reject the request, otherwise continue processing.
* OpenID Federation: If supported by the Wallet, the request is signed, and the `client_id` request parameter value is an HTTPS URI, the Wallet MUST the processing rules specified in OpenID Federation MUST be followed. When this is the case, the Entity Configuration for the Client is obtained at the path `<client_id>/.well-known/openid-federation`. Automatic Registration, as defined in OpenID Federation, MUST be used. The signed request MAY also contain a `trust_chain` parameter, which can optimize the process of obtaining metadata. The `jwks` parameter of the resolved Verifier metadata obtained from the Trust Chain after applying the policies, according to OpenID Federation, MUST be used to verify the signature of the signed request. The Wallet MUST verify that the `redirect_uri` or `response_uri` request parameter matches one of the entries of the `redirect_uris` parameter of the resolved metadata. If either of the checks fail, the Wallet MUST reject the request, otherwise processing continues.

If multiple of the above mechanisms are supported and apply, the Wallet SHOULD use the following order:
- If the request is signed, 
  ...
  - DID
  - OpenID Federation
  - Other specifications or ecosystem-specific support
- If the request is unsigned,
  - Pre-registered Clients: In addition to the rules above for pre-registered Clients, the Wallet MUST check that `require_request_object_signing` is set to `false` or omitted from the pre-registered Client metadata. The request MUST be rejected if `require_request_object_signing` is `true`, otherwise processing continues.
  - Client ID matches the Redirect and/or Response URI
...
Additionally, if the supports signed Request Objects, the Wallet SHOULD list supported cryptographic algorithms for securing the Request Object through the `request_object_signing_alg_values_supported` parameter. Conversely, the Wallet MUST NOT include this parameter if signed Request Objects are not supported.
...
Content-Type: application/x-www-form-urlencoded
  ...
  error_description=invalid%20request&
  state=eyJhb...6-sVA
```
...
The Verifier Attestation JWT is a JWT especially designed to allow a Wallet to authenticate a Verifier in a secure and flexible manner. A Verifier Attestation JWT is issued to the Verifier by a party that wallets trust for the purpose of authentication and authorization of Verifiers. The way this trust established is out of scope of this specification. Every Verifier is bound to a public key, the Verifier MUST always present a Verifier Attestation JWT along with the proof of possession for this key. In the case the Authorization Request is signed with this key, the Request Object serves as proof of possession.
...
# Document History
   ...
   -22
   * Removed `client_id_scheme` parameter
