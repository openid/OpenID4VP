...
* `authz_response_enc_pub_key`: OPTIONAL. The client's public key used as input to the key agreement for encryption of the authorization response in JWK [@!RFC7517] format. If not present, the key agreement is performed using a public key determined by the client's `jwks` or `jwks_uri` metadata parameter. The key agreement algorithm is determined by the `authorization_encrypted_response_alg` client metadata parameter. 
